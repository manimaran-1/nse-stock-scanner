...
def check_conditions(df, symbol):
    ...
    if is_intraday:
        ...
        indices_to_check = df.index[-75:] 
        
        # Further filter for today only
        indices_to_check = [idx for idx in indices_to_check if idx.date() == today_date]
    else:
...
def scan_market(symbols, interval='1d'):
    """
